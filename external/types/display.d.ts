...
declare namespace _pdfjs {
  ...
  let version: string;
  let build: string;
  let verbosity: VERBOSITY_LEVELS;
   ...
   * @var {string}
   */
  let cMapUrl: string | null;

  /**
   * Specifies if CMaps are binary packed.
   * @var {boolean}
   */
  let cMapPacked: boolean;
   ...
   * default: true
   */
  let postMessageTransfers: boolean;

  /**
   * Disables URL.createObjectURL usage.
   * @var {boolean}
   */
  let disableCreateObjectURL: boolean;
   ...
   * @var {number}
   */
  let externalLinkTarget: LinkTarget;
  ...
  function isExternalLinkTargetSet(): boolean;
